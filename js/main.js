...
function calculateAndShow() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none" style="animation:spin 0.8s linear infinite">
      <circle cx="7" cy="7" r="5" stroke="currentColor" stroke-width="1.5" stroke-dasharray="18 14"/>
    </svg> Berechne…`;
  }
  ...
  setTimeout(() => {
    ...
    if (btn) {
      ...
      btn.innerHTML = `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        ...
        <path d="M4 4h2v2H4zM8 4h2v2H8zM4 8h2v2H4zM8 8h2v2H8z" fill="currentColor"/>
      </svg> Plan berechnen`;
    }
...
function renderResults(r, inputs) {
  ...
  container.appendChild(el('div', { className: 'results-header' }, `
    <div class="results-profile">
      ...
      <p>${r.weight} kg · ${r.age} J · ${durationLabel} · ${r.intensityPct}% HR<sub>max</sub> · ${r.temperature}°C</p>
    </div>
    <div class="results-hr-info">
      <div class="hr-max-value">${r.hrMax} <span style="font-size:14px;color:var(--ink-3)">bpm</span></div>
      <div class="hr-max-label">HR<sub>max</sub> · TANAKA</div>
      <div style="margin-top:6px;font-family:var(--font-mono);font-size:11px;color:var(--ink-3);letter-spacing:0.04em">${r.zone.label}</div>
    </div>
  `));
  ...
  if (isMouthRinse) {
    container.appendChild(buildInfoPanel(
      'Belastung < 60 Min · Mouthrinse-Protokoll',
...
function buildRatioPanel(r) {
  ...
  return buildPanel(
    ...
    <div class="ratio-visual">
      <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--ink-3);margin-bottom:8px;">
...
function buildRatioExplanation(r) {
  ...
  if (paradigm === 'glucose_only') {
    return `<p style="font-size:13.5px;color:var(--ink-2);line-height:1.65">
  ...
  if (paradigm === '2_to_1') {
    return `<p style="font-size:13.5px;color:var(--ink-2);line-height:1.65">
      Das klassische <strong style="color:var(--ink)">2:1-Verhältnis</strong> sättigt SGLT1 optimal bei ~60 g/h Glukose
  ...
  if (paradigm === '1_to_0_8') {
    return `<p style="font-size:13.5px;color:var(--ink-2);line-height:1.65">
      Das moderne <strong style="color:var(--ink)">1:0.8-Paradigma</strong> (Rowlands, Podlogar et al.) maximiert bei
...
function buildTennisPanel(r) {
  ...
  return buildPanel(
    ...
    panelIconTennis(),
    `
    <div style="margin-bottom:20px;display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;">
    ...
    ${moreCount > 0 ? `<div style="text-align:center;padding:12px;font-size:11px;color:var(--ink-3)">… + ${moreCount} weitere Seitenwechsel im gleichen Schema</div>` : ''}
    <div style="margin-top:16px;padding:16px;background:color-mix(in oklab, var(--accent) 12%, transparent);border-radius:8px;border:1px solid rgba(59,158,255,0.2);font-size:11px;color:var(--ink-2);line-height:1.65">
      <strong style="color:var(--accent)">Darreichungsform:</strong>
...
function buildGutAdvisory(r) {
  ...
  panel.innerHTML = `
    <div class="panel-header">
      <div class="panel-icon" style="background:color-mix(in oklab, var(--warn) 12%, transparent);color:var(--warn)">
  ...
  return buildPanel(
    ...
    <div class="carb-loading-grid">
      <div class="carb-loading-day">
        ...
        <div style="font-size:11px;color:var(--ink-3);margin-top:4px">7–12 g/kg · Fettarm · Ballaststoffarm</div>
      </div>
      <div class="carb-loading-day">
        ...
        <div style="font-size:11px;color:var(--ink-3);margin-top:4px">8–12 g/kg · Leicht verdaulich</div>
      </div>
      <div class="carb-loading-day">
        <div class="day-label">${cl.morning_of.timing}</div>
        <div class="day-amount">${cl.morning_of.g} <span class="day-unit">g CHO</span></div>
        <div style="font-size:11px;color:var(--ink-3);margin-top:4px">2–4 g/kg · Leberglykogen optimieren</div>
      </div>
    </div>
    <div style="margin-top:16px;font-size:11px;color:var(--ink-3);line-height:1.65;padding:16px;background:var(--bg-elev);border-radius:8px">
...
function buildCaffeinePanel(r) {
  ...
  return buildPanel(
    ...
    <div style="margin-top:16px;font-size:13.5px;color:var(--ink-2);line-height:1.65;padding:16px;background:rgba(245,166,35,0.05);border:1px solid rgba(245,166,35,0.15);border-radius:8px">
      <strong style="color:var(--warn)">Timing:</strong> ${c.timing}<br/>
...
function buildInfoPanel(title, text, type) {
  ...
  panel.innerHTML = `
    ...
    <div class="panel-body">
      <p style="font-size:13.5px;color:var(--ink-2);line-height:1.65">${text}</p>
    </div>
  `;
  return panel;
}
